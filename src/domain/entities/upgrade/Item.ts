...
export default class Item {
    ...
    public getTypeCallback?: () => Promise<ItemType | undefined>;
    public getBrandCallback?: () => Promise<Brand | undefined>;
    public getImagesCallback?: () => Promise<ItemImage[]>;
    public getOrdersCallback?: () => Promise<OrderItem[]>;
    public getUsersCallback?: () => Promise<CartItem[]>;
    ...
    // Methods:
    public async getType(): Promise<ItemType | undefined> {
        if (!this.getTypeCallback) {
            return this.type;
        }

        if (!this.type) {
            this.type = await this.getTypeCallback();
        }
        else if ((this.type as any).fromConverter) {
            this.type = await this.getTypeCallback();
        }
    ...
    public async getBrand(): Promise<Brand | undefined> {
        if (!this.getBrandCallback) {
            return this.brand;
        }

        if (!this.brand) {
            this.brand = await this.getBrandCallback();
        }
        else if ((this.brand as any).fromConverter) {
            this.brand = await this.getBrandCallback();
        }
    ...
    public async getImages(): Promise<ItemImage[]> {
        if (this.images.length < 1 && this.getImagesCallback) {
            this.images = await this.getImagesCallback();
        }
        return this.images;
    }

    public async getOrders(): Promise<OrderItem[]> {
        if (this.orders.length < 1 && this.getOrdersCallback) {
            this.orders = await this.getOrdersCallback();
        }
        return this.orders;
    }

    public async getUsers(): Promise<CartItem[]> {
        if (this.users.length < 1 && this.getUsersCallback) {
            this.users = await this.getUsersCallback();
        }
        return this.users;
    }
