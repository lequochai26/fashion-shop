...
export default class ItemTypeManager extends PersistenceHandlerHolder implements SearchableEntityManager<ItemType,string>{
    //field
    private itemTypeConverter?: AsyncReversableConverter<ItemTypeData, ItemType> | undefined;
    private itemManager?: EntityManager<Item, string> | undefined;
    
    //constructor
    public constructor(
        persistenceHandler?: PersistenceHandler | undefined,
        itemTypeConverter?: AsyncReversableConverter<ItemTypeData,ItemType> | undefined,
    ...
    //private method
     private useItemtypeConverter<T>(
        executable:(itemTypeConverter: AsyncReversableConverter<ItemTypeData,ItemType>) => Promise<T>
    ): Promise<T> {
    ...
    private async setupDependencies(entity: ItemType, path: any[]): Promise<void>{
        ...
        entity.getItemsCallback = async function () {
            return self.useItemManager(
                async itemManager => itemManager.getByFilter({ type: entity.Id }, path)
            );
        }
    }
    public async get(pKey: string, path: any[]): Promise<ItemType | undefined> {
       ...
       entity = await this.useItemtypeConverter(
         async function (itemTypeConverter){
        ...
        return this.usePersistenceHandler(
           async function (persistenceHandler) {
                return persistenceHandler.insertItemType(
                    await self.useItemtypeConverter(
                        async function(itemTypeConverter){
    ...
    public set ItemTypeConverter(value: AsyncReversableConverter<ItemTypeData, ItemType> | undefined) {
